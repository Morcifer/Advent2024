...
fn part_1(file_path: String) -> u64 {
    ...
    for region in regions {
        ...
        let region_hashset = region.iter().copied().map(|(row, column)| (row as isize, column as isize)).collect::<HashSet<_>>();

        for (node_row, node_column) in region {
            let neighbour_count = [(0, 1), (1, 0), (0, -1), (-1, 0)]
                .into_iter()
                .map(|(d_row, d_column)| (node_row as isize + d_row, node_column as isize + d_column))
    ...
    for region in regions {
        ...
        for (node_row, node_column) in region {
            ...
            if !region_hashset.contains(&up_neighbour) {
                ...
                fence_edges.insert(((node_row, node_column), (node_row, node_column + 1)));
            }
            ...
            if !region_hashset.contains(&down_neighbour) {
                ...
                fence_edges.insert(((node_row + 1, node_column), (node_row + 1, node_column + 1)));
            }
            if !region_hashset.contains(&left_neighbour) {
                fence_points.insert((node_row, node_column));
                fence_points.insert((node_row + 1, node_column));
                fence_edges.insert(((node_row, node_column), (node_row + 1, node_column)));
            }

            if !region_hashset.contains(&right_neighbour) {
                fence_points.insert((node_row, node_column + 1));
                fence_points.insert((node_row + 1, node_column + 1));
                fence_edges.insert(((node_row, node_column + 1), (node_row + 1, node_column + 1)));
            }
        }
        ...
        while let Some(point) = fence_points_vec.pop() {
            ...
            while fence_edges.contains(&((left_point.0, left_point.1 - 1), left_point)) {
            ...
            while fence_edges.contains(&(right_point, (right_point.0, right_point.1 + 1))) {
            ...
            while fence_edges.contains(&((up_point.0 - 1, up_point.1), up_point)) {
            ...
            while fence_edges.contains(&(down_point, (down_point.0 + 1, down_point.1))) {
